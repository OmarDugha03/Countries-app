    ...
    <div className="flex flex-col items-center justify-center w-[95%] mx-auto  rounded-md md:grid md:grid-cols-2 lg:grid-cols-4 md:gap-8  ">
      {countries.map((i: any) => (
        <Link href={`/countries/${i.name.common}`} key={i.name.common}>
          <div className="flex flex-col max-w-md hover:shadow-[0px_0px_27px_4px_#a0aec0] transition-all duration-300 items-center my-2 text-left border rounded-lg shadow-xl cursor-pointer dark:border-slate-500 bg-slate-100 dark:bg-slate-800 ">
            <Suspense
              fallback={
